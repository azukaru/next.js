...
const NetworkRecorder = require('lighthouse/lighthouse-core/lib/network-recorder.js')
...
function headersArrayToHeadersDict(headersArray = []) {
  ...
  headersArray.forEach(headerItem => {
    const value =
      headersDict[headerItem.name] !== undefined
...
function willBeRedirected(networkRecords, record) {
  ...
  return networkRecords.some(
    otherRecord => otherRecord.requestId === redirectId
  )
}
...
function addRedirectResponseIfNeeded(networkRecords, record) {
  ...
  const originalRecord = networkRecords.find(
    record => record.requestId === originalId
  )
  if (!originalRecord) {
    throw new Error(
      `redirect with id ${record.requestId} has no original request`
    )
  }

  // populate `redirectResponse` with original's data, more or less.
  const originalResponse = getResponseReceivedEvent(originalRecord).params
    .response
 ...
 */
export default function networkRecordsToDevtoolsLog(
  networkRecords,
  options = {}
) {
  const devtoolsLog = []
  ...
  if (global.expect && !options.skipVerification) {
    const roundTrippedNetworkRecords = NetworkRecorder.recordsFromLogs(
      devtoolsLog
    )
